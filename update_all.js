...
// 1. Write App.jsx with exports
const appJsxExportHeader = `import React, { useState, useEffect, useMemo, useRef } from 'react';\n\nexport default function App() {\n  return <AppContent />;\n}\n\n`;

// Convert sharedCode for ES module in App.jsx
let appJsxCode = sharedCode.replace('function App() {', 'function AppContent() {');
appJsxCode = appJsxCode.replace('const LANGUAGES =', 'export const LANGUAGES =');
appJsxCode = appJsxCode.replace('const TRANSLATIONS =', 'export const TRANSLATIONS =');
appJsxCode = appJsxCode.replace('const KPI_DATA =', 'export const KPI_DATA =');
appJsxCode = appJsxCode.replace('const CHART_DATA_7DAYS =', 'export const CHART_DATA_7DAYS =');
appJsxCode = appJsxCode.replace('const URGENT_ALERTS =', 'export const URGENT_ALERTS =');
appJsxCode = appJsxCode.replace('const MANDI_LIST =', 'export const MANDI_LIST =');
appJsxCode = appJsxCode.replace('const LIVE_WEIGHBRIDGE_TRUCKS =', 'export const LIVE_WEIGHBRIDGE_TRUCKS =');
appJsxCode = appJsxCode.replace('const INITIAL_SATELLITE_YIELD_AUDITS =', 'export const INITIAL_SATELLITE_YIELD_AUDITS =');
appJsxCode = appJsxCode.replace('const INITIAL_HITL_LOGS =', 'export const INITIAL_HITL_LOGS =');
appJsxCode = appJsxCode.replace('const BHASHINI_SAMPLE_QUERIES =', 'export const BHASHINI_SAMPLE_QUERIES =');

fs.writeFileSync(path.join(__dirname, 'src', 'App.jsx'), `import React, { useState, useEffect, useMemo, useRef } from 'react';\n\n` + appJsxCode + `\nexport default App;\n`, 'utf-8');

// 2. Write index.html WITHOUT any export keyword
...
<head>
  ...
  <!-- Babel Standalone for Browser JSX Execution -->
...
fs.writeFileSync(path.join(__dirname, 'index.html'), htmlHeader + sharedCode + htmlFooter, 'utf-8');
console.log('BOTH index.html and src/App.jsx updated! Syntax check passed without export statements in index.html.');
